  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_ENVIRONMENT === 'production' ? 'http://mercatmediterrani.com:8001' : 'http://localhost:8001'}/users/${userId}/public-info`);
    ...
    try {
      ...
      const ticketPayload = {
        ...
        purchase_date: ticketData.purchase_date
              ...
              const response = await axios.post(`${process.env.REACT_APP_ENVIRONMENT === 'production' ? 'http://mercatmediterrani.com:8003' : 'http://localhost:8003'}/tickets/digital/`, ticketPayload);
